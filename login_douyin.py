...
class DouYinLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        # check login state
        logger.info(f"[DouYinLogin.begin] login finished then check login state ...")
        try:
    ...
    async def login_by_mobile(self):
        ...
        cache_client = CacheFactory.create_cache('menmory')
    ...
    async def check_page_display_slider(self, move_step: int = 10, slider_level: str = "easy"):
        ...
        while not slider_verify_success:
            ...
            try:
                ...
                logger.error(f"[DouYinLogin.check_page_display_slider] slider verify failed, error: {e}")
                await asyncio.sleep(1)
                max_slider_try_times -= 1
                logger.info(f"[DouYinLogin.check_page_display_slider] remaining slider try times: {max_slider_try_times}")
                continue
